...
class _CreateReservaScreenState extends State<CreateReservaScreen> {
  ...
  double get _costoTotal => _precioPorNoche * _noches * _numeroHuespedes;
  ...
  void _submitReserva() {
    ...
    const String currentUserId = 'mock_user_id'; // Reemplazar por Auth

    reservaBloc.add(CreateReserva(
      alojamientoId: widget.alojamientoData['id'] as String,
      ...
      body: BlocListener<ReservaBloc, ReservaState>(
        listener: (context, state) {
          if (state is ReservaCreationSuccess) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('¡Reserva creada con éxito! ID: ${state.reserva.id}'), backgroundColor: Colors.green),
            );
            Navigator.of(context).pop();
          } else if (state is ReservaCreationFailure) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Error al crear reserva: ${state.message}'), backgroundColor: Colors.red),
            );
          }
        },
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              if (_noches > 0) ...[
                ListTile(
                  title: Text('S/$_precioPorNoche x $_noches noche${_noches == 1 ? '' : 's'} x $_numeroHuespedes huésped${_numeroHuespedes == 1 ? '' : 's'}'),
                  trailing: Text('S/${_costoTotal.toStringAsFixed(2)}', style: textTheme.titleMedium),
                ),
                const ListTile(
                ...
                const Divider(),
                ListTile(
                  title: Text('Total Estimado', style: TextStyle(fontWeight: FontWeight.bold)),
                  trailing: Text('S/${(_costoTotal + 25.00).toStringAsFixed(2)} PEN', style: TextStyle(color: Colors.pink, fontSize: 18, fontWeight: FontWeight.bold)),
                ),
              ] else
